...
describe 'Scenario#4' do
  it 'Create a subscription for an existing account' do
    client = login
    ...
    account.bill_to_id = contact_id
    account.sold_to_id = contact_id
    account.status = 'Active'
    update_action = Zuora::Actions::Update.new [account]
    client.update update_action

    #use the existing account to create a subscribe request
    account = Zuora::Objects::Account.new
    account.id = account_id
