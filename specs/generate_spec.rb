...
describe 'Client' do
  context 'generate' do
    ...
    it 'should generate an invoice' do
      #generate an account and get its id
      ...
      #!!!use this account_id got from response, not the original account_id
      account_id = response[:subscribe_response][:result][:account_id].to_s



      #generate an invoice and pass account id to it
      invoice = Zuora::Objects::Invoice.new
      invoice.account_id = account_id
      invoice.includes_one_time = true
      invoice.includes_recurring = true
      invoice.includes_usage = true
      invoice.invoice_date = '2012-07-07T08:07:55-08:00'
      invoice.target_date = '2012-07-07T08:07:55-08:00'

      generate_action = Zuora::Actions::Generate.new([invoice])
      response = client.generate(generate_action)

      response[:generate_response][:result][:success].should == true
    end
  end
end
